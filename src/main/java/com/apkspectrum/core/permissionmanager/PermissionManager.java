...
public class PermissionManager {
    ...
    private void addAllPermissions() {
        XmlPath allPermissions =
                xmlPermissionDB.getNodeList("/permission-history/permissions/permission");
    ...
    public PermissionRecord getPermissionRecord(String name) {
        ...
        XmlPath permission = xmlPermissionDB
                .getNode("/permission-history/permissions/permission[@name='" + name + "']");
        if (permission == null) {
    ...
    public PermissionGroupRecord getPermissionGroupRecord(String name) {
        ...
        XmlPath node = xmlPermissionDB.getNode(
                "/permission-history/permission-groups/permission-group[@name='" + name + "']");
        if (node == null) {
    ...
    public PermissionGroupInfoExt[] getPermissionGroups(int sdk) {
            ...
            for (PermissionRecord record : recordMap.values()) {
                ...
                if (groupInfo == null) {
                    ...
                    if (groupRecord != null) {
                        ...
                        if (groupInfo != null
                                && (groupInfo.icon == null || groupInfo.icon.trim().isEmpty())) {
        ...
        PermissionGroupInfoExt[] result =
                groups.values().toArray(new PermissionGroupInfoExt[groups.size()]);
    ...
    public PermissionInfo[] getGroupPermissions(String groupName, int sdk) {
        ...
        if (GROUP_NAME_DECLARED.equals(groupName)) {
            for (DeclaredPermissionInfo declared : declaredMap.values()) {
                RevokedPermissionInfo reason;
                reason = RevokedPermissionInfo.makeRevokedReason(declared, sdk);
                boolean isGrant = reason.reason == RevokedReason.NO_REVOKED;
    ...
    public RevokedPermissionInfo makeRevokedReason(PermissionRecord record, int sdk) {
        return RevokedPermissionInfo.makeRevokedReason(record, sdk, targetSdkVersion,
                (!isPlatformSigned && isSignAsRevoked));
    }

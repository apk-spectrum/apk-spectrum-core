...
public class PermissionRepository {
    ...
    public class SourceCommit {
        ...
        public String getCommitId() {
            return commit != null && !commit.isEmpty() ? commit : null;
        }
    }

    public PermissionRepository(XmlPath sourcesNode) {
        this(sourcesNode.getAttribute("repository"), sourcesNode.getAttribute("manifest"),
                sourcesNode.getAttribute("resources"), sourcesNode.getAttribute("config"));
        sourcesNode = sourcesNode.getNodeList("source");
        for (int i = sourcesNode.getCount() - 1; i >= 0; --i) {
            sources.add(new SourceCommit(Integer.parseInt(sourcesNode.getAttribute(i, "sdk")),
                    sourcesNode.getAttribute(i, "commit")));
        }
    ...
    public PermissionRepository(String url, String manifestPath, String resourcePath,
            String config) {
        this.url = url;

...
public class OptionsBundle {
    ...
    public static final int FLAG_OPT_HAS_EXTRADATA_MASK =
            FLAG_OPT_INSTALL_LAUNCH | FLAG_OPT_PUSH_LIB32 | FLAG_OPT_PUSH_LIB64
                    | FLAG_OPT_PUSH_SYSTEM | FLAG_OPT_PUSH_PRIVAPP | FLAG_OPT_PUSH_OTHER;
    ...
    public static final int BLOACKED_INSTALL_MASK = BLOACKED_COMMON_CAUSE_UNSIGNED
            | BLOACKED_COMMON_CAUSE_UNSUPPORTED_SDK_LEVEL | BLOACKED_INSTALL_CAUSE_MISMATCH_SIGNED;
    public static final int BLOACKED_PUSH_MASK = BLOACKED_COMMON_CAUSE_UNSIGNED
            | BLOACKED_COMMON_CAUSE_UNSUPPORTED_SDK_LEVEL | BLOACKED_PUSH_CAUSE_NO_ROOT
            | BLOACKED_PUSH_CAUSE_MISMATCH_SIGNED_AND_NO_SYSTEM;
        ...
        switch (flag) {
            ...
            case FLAG_OPT_PUSH:
                if (isBlockedFlags(flag)) {
                    if ((blockedCause
                            & BLOACKED_COMMON_CAUSE_UNSIGNED) == BLOACKED_COMMON_CAUSE_UNSIGNED) {
                        cause = BLOACKED_COMMON_CAUSE_UNSIGNED;
                    } else if ((blockedCause
                            & BLOACKED_COMMON_CAUSE_UNSUPPORTED_SDK_LEVEL) == BLOACKED_COMMON_CAUSE_UNSUPPORTED_SDK_LEVEL) {
                        cause = BLOACKED_COMMON_CAUSE_UNSUPPORTED_SDK_LEVEL;
                    } else if (flag == FLAG_OPT_INSTALL) {
                        if ((blockedCause
                                & BLOACKED_INSTALL_CAUSE_MISMATCH_SIGNED) == BLOACKED_INSTALL_CAUSE_MISMATCH_SIGNED) {
                        ...
                        if ((blockedCause
                                & BLOACKED_PUSH_CAUSE_NO_ROOT) == BLOACKED_PUSH_CAUSE_NO_ROOT) {
                            cause = BLOACKED_PUSH_CAUSE_NO_ROOT;
                        } else if ((blockedCause
                                & BLOACKED_PUSH_CAUSE_MISMATCH_SIGNED_AND_NO_SYSTEM) == BLOACKED_PUSH_CAUSE_MISMATCH_SIGNED_AND_NO_SYSTEM) {
                            cause = BLOACKED_PUSH_CAUSE_MISMATCH_SIGNED_AND_NO_SYSTEM;
                        } else if ((blockedCause
                                & BLOACKED_PUSH_CAUSE_HAS_SU_BUT_NO_ROOT) == BLOACKED_PUSH_CAUSE_HAS_SU_BUT_NO_ROOT) {
    ...
    public synchronized void set(int flag, String... extraData) {
        ...
        if (flag == 0 || Integer.bitCount(flag) > 1) {
            // Log.v("Can be set flag to only one bit at once " + Integer.bitCount(flag) + ", " +
            // Integer.toHexString(flag));
            // return;
        }

        if ((flag & FLAG_OPT_HAS_EXTRADATA_MASK) != 0 && extraData == null) {
            Log.w("This flag is need extra Data, but extraData is null : 0x"
                    + Integer.toHexString(flag));
        }
    ...
    private void optionsChanged(int changedFlag, String... extraData) {
        ...
        synchronized (sLock) {
            listenersCopy = sChangedListeners
                    .toArray(new IOptionsChangedListener[sChangedListeners.size()]);
        }

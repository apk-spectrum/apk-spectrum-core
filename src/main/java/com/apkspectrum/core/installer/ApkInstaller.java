...
public class ApkInstaller {
    ...
    public void install(CompactApkInfo apkInfo, OptionsBundle options) {
        install(device, apkInfo, options);
    }

    public static String install(IDevice device, CompactApkInfo apkInfo, OptionsBundle options) {
        String errMessage = null;
        Log.i("InstallApk() device : " + device.getSerialNumber() + ", apkPath: "
                + apkInfo.filePath);
    ...
    private static String installApk(IDevice device, CompactApkInfo apkInfo,
        ...
        if (!extraArgs.isEmpty()) {
            Log.v("install options : " + extraArgs);
            argv = extraArgs.toArray(new String[extraArgs.size()]);
        }

        errMessage = PackageManager.installPackage(device, apkInfo.filePath, reinstall, argv);
        if (errMessage == null || errMessage.isEmpty()) {
            if (options.isSetLaunch()) {
                ...
                if (activity != null) {
                    ...
                    if (pacakgeName != null) {
                        String[] cmdResult = AdbDeviceHelper.launchActivity(device,
                                pacakgeName + "/" + activity);
                        if (cmdResult == null
                                || (cmdResult.length >= 2 && cmdResult[1].startsWith("Error"))
                                || (cmdResult.length >= 1 && cmdResult[0].startsWith("error"))) {
                            Log.e("activity start faile : " + pacakgeName + "/" + activity);

                            if (cmdResult != null) {
                                StringBuilder sb = new StringBuilder("cmd: adb shell start -n ")
                                        .append(pacakgeName).append("/").append(activity)
                                        .append("\n\n");
    ...
    private static String pushApk(IDevice device, CompactApkInfo apkInfo, OptionsBundle options) {
        String errMessage = null;
        ...
        if (!AdbDeviceHelper.hasSu(device)) {
            ...
            errMessage = "Permission denied: System is not root, "
                    + "try again after change root mode by 'adb root' command";
        } else {
        ...
        if (errMessage == null) {
            ...
            if (installedPath != null && installedPath.startsWith("/system/")) {
                String pattern = "^(/system/(priv-)?app/[^/]*/)[^/]*\\.apk";
                String removePath = installedPath.replaceAll(pattern, "$1");
        ...
        if (errMessage == null) {
            String tempPath = FileUtil.getTempPath() + File.separator + device.getSerialNumber()
                    + "_push" + File.separator;

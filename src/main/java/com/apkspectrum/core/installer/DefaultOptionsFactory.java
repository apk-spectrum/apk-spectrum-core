...
public class DefaultOptionsFactory {
    ...
    public DefaultOptionsFactory(CompactApkInfo apkInfo, SignatureReport signatureReport) {
        ...
        wasSigned = signatureReport != null;

        if (apkInfo == null) {
            return;
        }

        hasApkInfo = apkInfo.packageName != null && !apkInfo.packageName.isEmpty();
        wasSigned = wasSigned || apkInfo.certificates != null && apkInfo.certificates.length > 0;
        minSdkVersion = apkInfo.minSdkVersion != null ? apkInfo.minSdkVersion : 1;
    ...
    public OptionsBundle createOptions(IDevice device, OptionsBundle options) {
            ...
            if (signature != null && signature.matches("[0-9a-fA-F]*")) {
                if (!signatureReport.contains("RAWDATA", signature)) {
                    ...
                    if (!packageInfo.isSystemApp()) {
                        ...
                        blockedCause |=
                                OptionsBundle.BLOACKED_PUSH_CAUSE_MISMATCH_SIGNED_AND_NO_SYSTEM;
                    }
                }
            }
        }
        ...
        if (targetSystemPath == null) {
            SimpleOutputReceiver outputReceiver = new SimpleOutputReceiver();
            try {
                device.executeShellCommand("ls /system/app/*/*.apk; ls /system/app/*.apk",
                        outputReceiver);
            } catch (TimeoutException | ShellCommandUnresponsiveException | IOException e) {
                e.printStackTrace();
            } catch (AdbCommandRejectedException e1) {
            ...
            if (targetSystemPath.matches("^/system/(priv-)?app/[^/]*/[^/]*\\.apk")) {
                if (abi32 != null) {
                    options.set(OptionsBundle.FLAG_OPT_PUSH_LIB32, abi32,
                            targetSystemPath.replaceAll("[^/]*.apk$", "lib/arm/"));
                ...
                if (abi64 != null) {
                    options.set(OptionsBundle.FLAG_OPT_PUSH_LIB64, abi64,
                            targetSystemPath.replaceAll("[^/]*.apk$", "lib/arm64/"));
                ...
                String libDirectory =
                        targetSystemPath.replaceAll(".*/([^/]*).apk$", "/data/app-lib/$1/");
                if (abi32 != null) {

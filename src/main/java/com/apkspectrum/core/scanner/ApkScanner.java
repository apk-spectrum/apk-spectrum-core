...
abstract public class ApkScanner {
    ...
    public void openPackage(String devSerialNumber, String devApkFilePath, String framework) {
        if (statusListener != null) {
            statusListener.onProgress(1, "I: Open package\n");
            statusListener.onProgress(1, "I: apk path in device : " + devApkFilePath + "\n");
        }

        String tmpPath = "/" + devSerialNumber + devApkFilePath;
        ...
        if (framework != null && !framework.isEmpty()) {
            for (String s : framework.split(";")) {
                if (s.startsWith("@")) {
                    ...
                    String dest = (new File(tmpPath).getParent()) + File.separator
                            + path.replaceAll(".*/", "");
                    if (statusListener != null) {
                        statusListener.onProgress(1,
                                "I: start to pull resource apk " + path + "\n");
                    }
        ...
        if (statusListener != null) {
            statusListener.onProgress(1, "I: start to pull apk " + devApkFilePath + "\n");
        }
        boolean ret = AdbWrapper.pullApk(devSerialNumber, devApkFilePath, tmpPath,
        ...
        if (!ret || !(new File(tmpPath)).exists()) {
            Log.e("openPackage() failure : apk pull - " + tmpPath);
            if (statusListener != null) {
    ...
    public boolean isScanning() {
        synchronized (this) {
            return (lastErrorCode == NO_ERR && scanningStatus != 0
                    && scanningStatus != STATUS_ALL_COMPLETED);
        }
    }
    ...
    public String getErrorMessage(int errCode) {
        switch (errCode) {
            ...
            case ERR_NO_SUCH_FILE:
                return "No such apk file.";
            case ERR_NO_SUCH_MANIFEST:
                return "No such manifest file in the APK.";
            case ERR_FAILURE_PULL_APK:
                return "Can not pull apk from device.";
            case ERR_CAN_NOT_ACCESS_ASSET:
                return "Access fail for asset of APK.";
            case ERR_WRONG_MANIFEST:
                return "Manifest was wrong format.";
            case ERR_FAILURE_VERIFY_CERT:
                return "Bad signed.";
            case ERR_UNKNOWN:
            default:
    ...
    public static ApkScanner getInstance() {
        return ApkScanner
                .getInstance(_RProp.B.USE_EASY_UI.get() ? ApkScanner.APKSCANNER_TYPE_AAPTLIGHT
                        : ApkScanner.APKSCANNER_TYPE_AAPT);
    }
    ...
    public static String getPackageName(final String apkFilePath) {
        if (apkFilePath == null || apkFilePath.isEmpty() || !new File(apkFilePath).isFile()) {
            Log.e("No such file " + apkFilePath);
            return null;
        }

        Log.i("getDump AndroidManifest...");
        String[] androidManifest = AaptNativeWrapper.Dump.getXmltree(apkFilePath,

...
public class ApkInfoHelper {
    ...
    public static boolean hasLauncher(ApkInfo apkInfo) {
        return (apkInfo != null && (apkInfo.manifest.featureFlags
                            & ManifestInfo.MANIFEST_FEATURE_LAUNCHUR) != 0);
    }
    ...
    public static boolean isStartup(ApkInfo apkInfo) {
        return (apkInfo != null && (apkInfo.manifest.featureFlags
                            & ManifestInfo.MANIFEST_FEATURE_STARTUP) != 0);
    }

    public static boolean isInstrumentation(ApkInfo apkInfo) {
        return (apkInfo != null && (apkInfo.manifest.featureFlags
                        & ManifestInfo.MANIFEST_FEATURE_INSTRUMENTATION) != 0);
    }

    public static boolean isDebuggable(ApkInfo apkInfo) {
        return (apkInfo != null
                && apkInfo.manifest.application.debuggable != null
                && apkInfo.manifest.application.debuggable);
    }

    public static boolean isSigned(ApkInfo apkInfo) {
        return (apkInfo != null && apkInfo.certificates != null
                && apkInfo.certificates.length > 0);
    }
        ...
        if (apkInfo != null) {
            if (apkInfo.manifest.application.activity != null) {
                for (ActivityInfo info: apkInfo.manifest.application.activity) {
                    ...
                    if (exported == null) {
                        exported = info.intentFilter != null
                                && info.intentFilter.length > 0;
                    }
    ...
    public static String getResourceValue(ResourceInfo[] resInfos
            ...
            for (String preferred: preferredLanguage) {
                ...
                for (ResourceInfo r: resInfos) {
                    if (r.configuration != null && !r.configuration.isEmpty()
                            ...
                            && r.configuration.equalsIgnoreCase(preferred)
                            && r.name != null) {
                        value = r.name;
                        break;
                    }

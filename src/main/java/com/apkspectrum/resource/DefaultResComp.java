...
public class DefaultResComp implements ResComp<ResComp<?>> {
    ...
    public DefaultResComp(ResString<?> text, ResImage<?> image, ResString<?> toolTipText) {
        this(text, (Icon) null, toolTipText);
        this.image = image;
    }

    public DefaultResComp(ResString<?> text, Icon icon, ResString<?> toolTipText) {
        this.text = text;
        this.icon = icon;
        ...
        if (window == null) {
            c.addHierarchyListener(new HierarchyListener() {
                ...
                public void hierarchyChanged(HierarchyEvent e) {
                    if (e.getID() != HierarchyEvent.HIERARCHY_CHANGED) return;

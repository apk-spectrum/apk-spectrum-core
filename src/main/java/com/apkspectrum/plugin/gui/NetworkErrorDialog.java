...
public class NetworkErrorDialog {
    ...
    private static final String IGNORE_NO_SUCHE_INTERFACE =
            PlugInConfig.CONFIG_IGNORE_NETWORK_ERR_NO_SUCHE_INTERFACE;

    private static final String IGNORE_TIME_OUT =
            PlugInConfig.CONFIG_IGNORE_NETWORK_ERR_CONNECTION_TIMEOUT;

    private static final String IGNORE_SSH_HANDSHAKE =
            PlugInConfig.CONFIG_IGNORE_NETWORK_ERR_SSL_HANDSHAKE;
    ...
    public static int show(Component parent, UpdateChecker plugin, boolean force) {
        ...
        if (pluginName == null) {
            pluginName = "IUpdateChecker@0x" + Integer.toHexString(plugin.hashCode());
        }
        ...
        if (e.isNetworkNotFoundException()) {
            ...
            if (!force) {
                config.setConfiguration(IGNORE_NO_SUCHE_INTERFACE,
                        errPanel.isNeverLook() ? "true" : "false");
            }
            ...
            if (!force) {
                config.setConfiguration(IGNORE_TIME_OUT, errPanel.isNeverLook() ? "true" : "false");
            }
            PlugInManager.saveProperty();
            ...
            if (!force) {
                config.setConfiguration(IGNORE_SSH_HANDSHAKE,
                        errPanel.isNeverLook() ? "true" : "false");
            }
            PlugInManager.saveProperty();
            // boolean ignoreSSL = "true".equals(
            // updater.getPlugInPackage().getConfiguration(
            // PlugInConfig.CONFIG_IGNORE_SSL_CERT, "false", true));
        } else {
    ...
    private static int showOptionDialog(Component parentComponent, Object context, String title) {
        return MessageBoxPane.showOptionDialog(parentComponent, context, title,
                JOptionPane.DEFAULT_OPTION, MessageBoxPane.ERROR_MESSAGE, null,
                new String[] {_RStr.BTN_RETRY.get(), _RStr.BTN_CLOSE.get()}, _RStr.BTN_CLOSE.get());
    }
}

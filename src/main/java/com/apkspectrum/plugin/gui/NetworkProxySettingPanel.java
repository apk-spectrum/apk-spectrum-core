...
public class NetworkProxySettingPanel extends JPanel implements ActionListener {
    ...
    public NetworkProxySettingPanel(PlugInPackage pluginPackage) {
        ...
        String[] proxyMethods = new String[] {_RStr.PROXY_MENU_NO_PROXY.get(),
                _RStr.PROXY_MENU_GLOBAL.get(), _RStr.PROXY_MENU_SYSTEM.get(),
                _RStr.PROXY_MENU_PAC_SCRIPT.get(), _RStr.PROXY_MENU_MANUAL.get()};
    ...
    private JPanel makeProxyFileds() {
        ...
        GridBagConstraints proxyIpConst =
                new GridBagConstraints(1, 0, 1, 1, 0.7, 0, GridBagConstraints.WEST,
                        GridBagConstraints.HORIZONTAL, new Insets(10, 0, 0, 0), 0, 0);
        GridBagConstraints proxyPortConst =
                new GridBagConstraints(2, 0, 1, 1, 0.3, 0, GridBagConstraints.WEST,
    ...
    public void setPluginPackage(PlugInPackage pluginPackage) {
        ...
        boolean useGlobalConfig = "true".equals(
                pluginConfig.getConfiguration(PlugInConfig.CONFIG_USE_GLOBAL_PROXIES, "true"));

        boolean useSystemProxy =
                "true".equals(pluginConfig.getConfiguration(PlugInConfig.CONFIG_USE_SYSTEM_PROXIES,
                        useGlobalConfig ? "true" : "false"));

        boolean usePacProxy = "true".equals(
                pluginConfig.getConfiguration(PlugInConfig.CONFIG_USE_PAC_PROXIES, "false"));

        boolean noProxy = "true"
                .equals(pluginConfig.getConfiguration(PlugInConfig.CONFIG_NO_PROXIES, "false"));
        ...
        proxySet[2]
                .setText(pluginConfig.getConfiguration(PlugInConfig.CONFIG_HTTPS_PROXY_HOST, ""));
        proxySet[3]
                .setText(pluginConfig.getConfiguration(PlugInConfig.CONFIG_HTTPS_PROXY_PORT, ""));

        // proxySet[4].setText(pluginConfig.getConfiguration(
        // PlugInConfig.CONFIG_HTTP_PROXY_HOST, ""));
        // proxySet[5].setText(pluginConfig.getConfiguration(
        // PlugInConfig.CONFIG_HTTP_PROXY_HOST, ""));
        // proxySet[6].setText(pluginConfig.getConfiguration(
        // PlugInConfig.CONFIG_HTTP_PROXY_HOST, ""));
        // proxySet[7].setText(pluginConfig.getConfiguration(
        // PlugInConfig.CONFIG_HTTP_PROXY_HOST, ""));
    }
    ...
    public void actionPerformed(ActionEvent e) {
        if (!ACT_CMD_APPLY.equals(e.getActionCommand())) return;
        ...
        switch (methodCardMap.get(methods.getSelectedItem())) {
            ...
            case CARD_MANUAL_PROXY:
                pluginConfig.setConfiguration(PlugInConfig.CONFIG_HTTP_PROXY_HOST,
                        proxySet[0].getText());
                pluginConfig.setConfiguration(PlugInConfig.CONFIG_HTTP_PROXY_PORT,
                        proxySet[1].getText());
                pluginConfig.setConfiguration(PlugInConfig.CONFIG_HTTPS_PROXY_HOST,
                        proxySet[2].getText());
                pluginConfig.setConfiguration(PlugInConfig.CONFIG_HTTPS_PROXY_PORT,
                        proxySet[3].getText());
                break;
            default:
                break;

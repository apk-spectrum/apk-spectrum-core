...
public class NetworkSetting
{
	...
	public static final String APK_SCANNER_SSL_TRUSTSTORE
	...
	static {
		Logger.setBackend(new LogBackEnd() {
			@Override
			public void log(Class<?> clzz, LogLevel level,
	...
	public static void setProxyServer(PlugInPackage pluginPackage, URI uri) {
		...
		boolean useGlobalConfig = "true".equals(config.getConfiguration(
				PlugInConfig.CONFIG_USE_GLOBAL_PROXIES, "true"));
		if(useGlobalConfig) config.setPlugInPackage(null);

		boolean useSystemProxy = "true".equals(config.getConfiguration(
										PlugInConfig.CONFIG_USE_SYSTEM_PROXIES,
										useGlobalConfig ? "true" : "false"));
		boolean usePacProxy = "true".equals(config.getConfiguration(
										PlugInConfig.CONFIG_USE_PAC_PROXIES,
										"false"));

		boolean wasSetProxy = false;
		if((useSystemProxy || usePacProxy)
				&& SystemUtil.checkJvmVersion("1.8")) {
			...
			if(useSystemProxy) {
	            ...
	            proxySearch.setPacCacheSettings(20, 1000*60*10,
	            		CacheScope.CACHE_SCOPE_URL);
	            proxySelector = proxySearch.getProxySelector();
	            if(proxySelector != null) {
	            	l = proxySelector.select(uri);
	            }
			} else {
    			String pacUrl = config.getConfiguration(
    										PlugInConfig.CONFIG_PAC_URL, "");
    			if (pacUrl.startsWith("file://")
    					&& !pacUrl.startsWith("file:///")) {
            ...
            if(l != null) for (Proxy proxy: l) {
                ...
                if(addr != null) {
                	...
                	System.setProperty(uri.getScheme() + ".proxyHost",
                						addr.getHostName());
			...
			Log.w("Can't supported that get system proxy setting under on "
					+ "JVM 1.7 or earlier");
		}

		if(!wasSetProxy) {
			boolean noProxy = "true".equals(config.getConfiguration(
									PlugInConfig.CONFIG_NO_PROXIES, "false"));
			if(!noProxy) {
		...
		try {
			...
			while (interfaces.hasMoreElements()) {
			    ...
			    if (interf.isUp() && !interf.isLoopback()) {
				    ...
				    for (Iterator<?> iter = adrs.iterator();
				    		iter.hasNext();) {
			...
			if(trustStore == null) {
			    ...
			    ts.load(new FileInputStream(new File(trustStore)),
			    		config.getConfiguration(
			    				PlugInConfig.CONFIG_SSL_TRUSTSTORE_PWD,

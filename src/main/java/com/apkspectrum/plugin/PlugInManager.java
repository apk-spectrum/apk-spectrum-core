...
public final class PlugInManager {
    ...
    public static void setAppPackage(String appPackageName, String appVerseion, Res<String> appName,
            Res<Image> appIcon) {
    ...
    public static UpdateChecker[] getUpdateChecker() {
        List<UpdateChecker> list = new ArrayList<>();
        for (PlugInPackage pack : pluginPackages) {
    ...
    public static ExternalTool[] getExternalTool() {
        List<ExternalTool> list = new ArrayList<>();
        for (PlugInPackage pack : pluginPackages) {
            for (PlugIn p : pack.getPlugIn(PlugIn.PLUGIN_TPYE_EXTERNAL_TOOL)) {
                if (p instanceof ExternalTool && p.isEnabled() && ((ExternalTool) p).isSupoortedOS()
    ...
    public static PackageSearcher[] getPackageSearchers() {
        List<PackageSearcher> list = new ArrayList<>();
        for (PlugInPackage pack : pluginPackages) {
    ...
    public static ExtraComponent<?>[] getExtraComponenet() {
        List<ExtraComponent<?>> list = new ArrayList<>();
        for (PlugInPackage pack : pluginPackages) {
    ...
    public static PlugIn getPlugInByActionCommand(String actionCommand) {
        ...
        return pack == null ? null : pack.getPlugInByActionCommand(actionCommand);
    }
    ...
    public static ApkInfo getApkInfoByEventSource() {
        return actionHandler != null ? actionHandler.getApkInfoByEventSource() : null;
    }

    public static ApkInfo getApkInfoByEventSource(EventObject e) {
        return actionHandler != null ? actionHandler.getApkInfoByEventSource(e) : null;
    }
        ...
        synchronized (sLock) {
            ...
            for (File pluginFile : plugins) {
                ...
                if (pack != null) {
                    ...
                    minVer = GeneralVersionChecker.parseFrom(pack.getMinCoreVersion());
                    if (coreVer.compareTo(minVer) < 0) {
                        Log.w(packageName + " requires core version " + minVer + " or higher.");
                        continue;
                    }
                    PlugInPackage oldPack = getPlugInPackage(packageName);
                    if (oldPack != null) {
                        Log.i(packageName + " was already existed " + "a same package : "
                        ...
                        if (oldPack.getVersionCode() < pack.getVersionCode()) {
                            ...
                            Log.i("This is old version or same, so do not add " + "a package : "
                                    + pack.getPlugInUri());
                        }
                    } else {
                        pluginPackages.add(pack);
            ...
            synchronized (sLock) {
                listenersCopy =
                        eventListeners.toArray(new PlugInEventListener[eventListeners.size()]);
            }
    ...
    public static void checkForUpdatesWithUI(final Component parent, final long delayMs) {
    ...
    private static void checkForUpdates(final Component parent, final UpdateChecker[] updater) {
        new SwingWorker<UpdateChecker[], UpdateChecker>() {
            @Override
            protected UpdateChecker[] doInBackground() throws Exception {
                List<UpdateChecker> newUpdates = new ArrayList<>();
            ...
            protected void process(List<UpdateChecker> updater) {
                ...
                List<UpdateChecker> retryUpdates = new ArrayList<>();
                for (UpdateChecker uc : updater) {
                ...
                if (!retryUpdates.isEmpty()) {
                    checkForUpdates(parent,
                            retryUpdates.toArray(new UpdateChecker[retryUpdates.size()]));
                }
            }
            ...
            protected void done() {
                ...
                if (updaters != null && parent != null) {
                    if (!"true".equals(PlugInConfig
                            .getGlobalConfiguration(PlugInConfig.CONFIG_NO_LOOK_UPDATE_POPUP))) {
                ...
                if (updaters != null) {
                    ...
                    PlugInEventListener[] listenersCopy = null;
                    synchronized (sLock) {
                        listenersCopy = eventListeners
                                .toArray(new PlugInEventListener[eventListeners.size()]);
                    }

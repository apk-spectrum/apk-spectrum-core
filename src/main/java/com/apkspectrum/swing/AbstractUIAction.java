...
public abstract class AbstractUIAction extends AbstractAction implements UIAction {
    ...
    public void putValue(String key, Object newValue) {
        switch (key) {
            case ACTION_EVENT_HANDLER:
                setHandler(newValue instanceof ActionEventHandler ? (ActionEventHandler) newValue
                        : null);
                break;
            case ACTION_REQUIRED_CONDITIONS:
                setRequiredConditions(
                        newValue instanceof Integer ? ((Integer) newValue).intValue() : 0);
                break;
            default:
    ...
    public String getActionCommand() {
        ...
        if (actCmd == null) {
            try {
                actCmd = (String) getClass().getDeclaredField(ACTION_COMMAND_FIELD).get(null);
            } catch (Exception e) {
            }
        }
    ...
    public Icon getLargeIcon() {
        ...
        return ImageScaler.getScaledImageIcon((ImageIcon) icon, ((Dimension) size).width,
    ...
    public void setDisplayedMnemonicIndex(int index) {
        putValue(DISPLAYED_MNEMONIC_INDEX_KEY, Integer.valueOf(index));
    }

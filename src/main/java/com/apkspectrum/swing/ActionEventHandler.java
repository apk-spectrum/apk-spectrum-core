...
public class ActionEventHandler implements ActionListener, PropertyChangeListener {
    ...
    public Class<? extends Enum<? extends ResAction<?>>> getDefaultActionResourceClass() {
        return defRes;
    }
    ...
    public void loadActions(String packageName,
        ...
        for (Class<?> cls : classes) {
            if (cls.isMemberClass() || cls.isInterface() || Modifier.isAbstract(cls.getModifiers())
                    || !ActionListener.class.isAssignableFrom(cls))
                continue;
    ...
    private ResAction<?> getResAction(Class<? extends Enum<? extends ResAction<?>>> actRes,
    ...
    private void addActionListener(String actionCommand, ActionListener action,
        ...
        if (actionMap.containsKey(actionCommand)) {
            Log.v(String.format("addAction() %s was already existed. " + "Change to new : %s",
    ...
    public void setFlag(int flag) {
        ...
        if ((oldValue & flag) != flag) {
            this.flags |= flag;
            updateActionStatus();
            firePropertyChange(CONDITIONS_KEY, Integer.valueOf(oldValue),
    ...
    public void putData(String key, Object newValue) {
        if (CONDITIONS_KEY.equals(key)) {
            setFlag(!(newValue instanceof Integer) ? 0 : ((Integer) newValue).intValue());
            return;
        }
    ...
    private String getActionCommand(ActionListener action) {
        ...
        String actCmd = null;
        if (action instanceof Action) {
            actCmd = (String) ((Action) action).getValue(Action.ACTION_COMMAND_KEY);
        }
        if (actCmd == null) {
            try {
                actCmd = (String) action.getClass().getDeclaredField(UIAction.ACTION_COMMAND_FIELD)
                        .get(null);
            } catch (Exception e) {
            }
        }
    ...
    protected void firePropertyChange(String propertyName, Object oldValue, Object newValue) {
        if (pcs == null || (oldValue != null && newValue != null && oldValue.equals(newValue))) {
            return;
        }

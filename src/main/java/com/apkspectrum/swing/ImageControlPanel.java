...
public class ImageControlPanel extends JPanel implements ActionListener {
    ...
    public ImageControlPanel() {
        ...
        JScrollPane imageInfoScroll = new JScrollPane(imageInfoPanel,
        ...
        scrollpanel.addMouseMotionListener(new MouseAdapter() {
            public void mouseDragged(MouseEvent e) {
                ...
                JViewport viewPort =
                        (JViewport) SwingUtilities.getAncestorOfClass(JViewport.class, scrollpanel);
    ...
    private class ImageViewPanel extends JPanel implements MouseListener {
        ...
        public void setImage(Image image) {
            ...
            ViewPortsize = new Dimension(scroll.getViewport().getWidth() - 10,
                    scroll.getViewport().getHeight() - 10);

            // ViewPortsize.setSize(new Dimension((int)ViewPortsize.getWidth()-10
            // , (int)ViewPortsize.getHeight()-10));

            if (ViewPortsize.getWidth() < image.getWidth(this)
                    || ViewPortsize.getHeight() < image.getHeight(this)) {
                scale = (float) ((ViewPortsize.getWidth() / (image.getWidth(this)) <= ViewPortsize
                        .getHeight() / (image.getHeight(this)))
                                ? (ViewPortsize.getWidth() / image.getWidth(this))
                                : (ViewPortsize.getHeight() / image.getHeight(this)));
                // DefalutMinscale = scale;
            } else {

            }
            // Log.d("DefalutMinscale : " + DefalutMinscale);
            // Log.d("ViewPortsize.getWidth() : "+ViewPortsize.getWidth()
            // +" ViewPortsize.getHeight() = "+ ViewPortsize.getHeight());
        ...
        public void paintComponent(Graphics g) {
            ...
            if (bi != null) {
                ...
                bgbi = new BufferedImage(imageBackground.getWidth(this),
                        imageBackground.getHeight(this), BufferedImage.TYPE_INT_ARGB);
                bgbi.createGraphics().drawImage(imageBackground, 0, 0, this);

                paint = new TexturePaint(bgbi,
                        new Rectangle(0, 0, bgbi.getWidth(), bgbi.getHeight()));

                g2D.setPaint(paint);
                // g2D.fillRect((int)positionx, (int)positiony, (int)(bi.getWidth()* scale)
                // , (int)(bi.getHeight() * scale));

                Rectangle2D rect = new Rectangle2D.Double(0, 0, (bi.getWidth() * scale),
                        (bi.getHeight() * scale));

                // og.d("positionx : " + positionx + " positiony : "
                // + positiony + "scale : " + scale);
                // Log.d("bi.getWidth()* scale : " + bi.getWidth()* scale
                // + " bi.getHeight() * scale : " +(bi.getHeight() * scale) );
                ...
                g2D.drawImage(bi, (int) 0, (int) 0, (int) (bi.getWidth() * scale),
                        (int) (bi.getHeight() * scale), this);
                ...
                // Log.d("ViewPortsize.getWidth() : "+ViewPortsize.getWidth()
                // +" ViewPortsize.getHeight() = "+ ViewPortsize.getHeight());
                // Log.d("(int)(bi.getWidth()*scale : "+bi.getWidth()*scale
                // +" bi.getHeight()*scale = "+ bi.getHeight()*scale);
                // scrollRectToVisible(new Rectangle((int)(tempx+x)
                // ,(int)(tempy+y),(int)(ViewPortsize.getWidth()),(int)(ViewPortsize.getHeight())));

...
public class AdbDeviceHelper {
    ...
    public static boolean isShowingLockscreen(IDevice device) {
        ...
        try {
            device.executeShellCommand("dumpsys window policy | grep mShowingLockscreen"
    ...
    public static Dimension getPhysicalScreenSize(IDevice device) {
        final StringBuilder shResultBuilder = new StringBuilder();
        try {
            device.executeShellCommand("wm size | grep 'Physical size:'"
                    , new IShellOutputReceiver() {
                @Override
                public void addOutput(byte[] arg0, int arg1, int arg2) {
                    shResultBuilder.append(new String(arg0));
    ...
    public static boolean isPosibleDismissKeyguard(IDevice device) {
        final StringBuilder shResultBuilder = new StringBuilder();
        try {
            device.executeShellCommand("wm | grep 'wm dismiss-keyguard'"
                    , new IShellOutputReceiver() {
                @Override
                public void addOutput(byte[] arg0, int arg1, int arg2) {
                    shResultBuilder.append(new String(arg0));
    ...
    public static boolean isScreenOn(IDevice device) {
        final StringBuilder shResultBuilder = new StringBuilder();
        try {
            device.executeShellCommand("dumpsys input_method | grep -i 'mActive='"
                    , new IShellOutputReceiver() {
                @Override
                public void addOutput(byte[] arg0, int arg1, int arg2) {
                    shResultBuilder.append(new String(arg0));
    ...
    public static String[] launchActivity(IDevice device, String activity) {
        ...
        try {
            device.executeShellCommand("am start -n " + activity
                    , new IShellOutputReceiver() {
                @Override
                public void addOutput(byte[] arg0, int arg1, int arg2) {
    ...
    public static void tryDismissKeyguard(IDevice device) {
        ...
        if (!AdbDeviceHelper.isPosibleDismissKeyguard(device)
            ...
            if (screenSize != null) {
                ...
                unlockCmd = String.format("input touchscreen swipe %d %d %d %d;"
                        , 0, y, screenSize.width, y);
            }
        }
        ...
        if (apiLevel >= 20) {
            ...
            try {
                device.executeShellCommand("input keyevent KEYCODE_WAKEUP;"
                                + unlockCmd + "input keyevent KEYCODE_MENU;"
                        , new IShellOutputReceiver() {
                    @Override
                    public void addOutput(byte[] arg0, int arg1, int arg2) {
                    }
            ...
            Log.v("device api level " + apiLevel);
            String powerKeyCmd = "";
            if (!AdbDeviceHelper.isScreenOn(device)) {
                powerKeyCmd = "input keyevent KEYCODE_POWER;";
            }
            try {
                device.executeShellCommand(powerKeyCmd + unlockCmd
